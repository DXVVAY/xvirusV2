...
def getCaptchaBalance():
    return float(Captcha.getCapBal().strip('$'))
...
def token_joiner():
    ...
    start_balance = getCaptchaBalance()
    ...
    if tokens:
        ...
        print(f" {status}")
        end_balance = getCaptchaBalance()
        Output("info", config).notime(f"Captcha Info:")
        print(f" {Fore.LIGHTCYAN_EX}Start: {start_balance:.1f} {Fore.LIGHTBLACK_EX}| {Fore.RED}Used: {start_balance - end_balance:.1f} {Fore.LIGHTBLACK_EX}| {Fore.MAGENTA}End: {end_balance:.1f}\n")
        Output.PETC()
    else:
